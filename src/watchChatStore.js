...
function normalizeAnalysis(value) {
  ...
  const items = Array.isArray(raw.items)
    ? raw.items.slice(0, 10).filter((item) => /^\d{6}$/.test(String(item?.code || ''))).map((item) => ({
        ...
        allocation_status: String(item.allocation_status || '').slice(0, 24),
        allocation_reference_pct: Number(item.allocation_reference_pct) || 0,
        ...
        minimum_lot_weight: Number(item.minimum_lot_weight) || 0,
        execution_note: String(item.execution_note || '').slice(0, 240),
        long_term_zone: item.long_term_zone && typeof item.long_term_zone === 'object' ? {
          label: String(item.long_term_zone.label || '').slice(0, 24),
          lower: Number(item.long_term_zone.lower) || 0,
          upper: Number(item.long_term_zone.upper) || 0,
          basis: String(item.long_term_zone.basis || '').slice(0, 180),
          confirmation: String(item.long_term_zone.confirmation || '').slice(0, 240),
          invalidation: String(item.long_term_zone.invalidation || '').slice(0, 200),
          note: String(item.long_term_zone.note || '').slice(0, 160),
          scope: String(item.long_term_zone.scope || '').slice(0, 240),
        } : null,
